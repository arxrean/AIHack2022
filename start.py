...
class LeftModel(torch.nn.Module):
	def __init__(self, **kwargs):
		...
		self.add_module("remaining_block", torch.nn.Sequential(*[
			torch.nn.BatchNorm3d(left_config.get("c9").get("in_feat")),
			torch.nn.ConvTranspose3d(left_config.get("c9").get("in_feat"),
									 left_config.get(
						"c9").get("out_feat"),
	...
	def __init__(self, **kwargs):
		...
		for key in list(right_config.keys()):
			...
			if key in ["begin", "end"]:
				...
				conv.append(Conv3dCustom(**config))
			elif key == "up":
...
if __name__ == '__main__':
	...
	right_config = dict(
		...
		up=dict(size=64, mode='trilinear', align_corners=False),
	...
	l_model = LeftModel(left_config=left_config).cuda()
	r_model = RightModel(right_config=right_config).cuda()

	dataset = Dataset(data)
	dataloader = torch.utils.data.DataLoader(
		dataset, batch_size=1, shuffle=True, collate_fn=None)

	# print(l_model(next(iter(dataloader))[:, 0, ...]).size())
	# print(r_model(next(iter(dataloader))[:, 0, ...]).size())
	for _ in range(40):
		for step, pack in enumerate(dataloader):
			x = pack[:, 0, ...].cuda()
			out1 = l_model(x)
			out2 = r_model(x)